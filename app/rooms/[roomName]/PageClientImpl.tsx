...
type LobbyState =
  | { status: 'prejoin' }
  | { status: 'checking' }
  | { status: 'waiting'; displayName: string; lobbyToken: string; serverUrl: string }
  ...
  const handlePreJoinSubmit = React.useCallback(async (values: LocalUserChoices) => {
    ...
    if (lobbyData.isHost || !lobbyData.needsLobby) {
      ...
      setLobbyState({
        ...
        lobbyToken: lobbyData.lobbyToken,
        serverUrl: lobbyData.serverUrl,
      });
    }
  ...
  if (lobbyState.status === 'waiting') {
    return (
      <LobbyScreen
        ...
        lobbyToken={lobbyState.lobbyToken}
        serverUrl={lobbyState.serverUrl}
    ...
    <div className="lk-room-container" style={{ display: 'flex', height: '100dvh', width: '100dvw', overflow: 'hidden' }}>
      <RoomContext.Provider value={room}>
        ...
        <button
          ...
          className="fixed top-4 right-4 z-40 flex items-center gap-2 px-3 py-2 rounded-[2px] bg-surface border border-white/12 font-mono text-[11px] text-white/60 hover:text-white hover:bg-white/5 transition-colors"
        >
